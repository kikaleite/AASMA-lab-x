...
public class Board {
	...
	public static void initialize() {
		...
		for(int i=rampX, k=0; i<2*rampX; i++) {
			for(int j=0; j<rampY; j++) {
				board[i-1][j] = new Block(Shape.ramp, Color.gray);
				if((j==0||j==1) && (i==(rampX+1)||i==(rampX+2))) continue;
				else boxes.add(new Box(new Point(i-1,j), colors[k++%4]));
			}
		}
	...
	public static void insertBox(Box box, Point point) {
		objects[point.x][point.y] = box;
		boxes.add(box);
	}
	public static void removeBox(Point point) {
		boxes.remove((Box)objects[point.x][point.y]);
		objects[point.x][point.y] = null;
	}
